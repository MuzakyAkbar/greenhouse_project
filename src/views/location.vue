...
onMounted(async () => {
  // Parse location_id dari route params
  const paramId = route.params.location_id
  console.log('🔍 Route params:', route.params)
  console.log('🔍 Param location_id:', paramId, 'Type:', typeof paramId)
  
  if (!paramId) {
    console.error('❌ No location_id in route params')
    alert('Location ID tidak ditemukan')
    router.push('/dashboard')
    return
  }
  
  locationId.value = parseInt(paramId, 10)
  
  if (isNaN(locationId.value)) {
    console.error('❌ Invalid location_id (NaN)')
    alert('Location ID tidak valid')
    router.push('/dashboard')
    return
  }
  
  console.log('✅ Valid location_id:', locationId.value)
...
const loadLocationData = async () => {
  console.log('📍 Loading location data for ID:', locationId.value)
  ...
  console.log('✅ Location loaded:', locData)
...
const loadProductionData = async () => {
  console.log('📊 Loading production data for location:', locationId.value)
  ...
  console.log('📦 Production data loaded:', productionData?.length || 0, 'records')
  ...
  console.log('📋 Report data loaded:', reportData?.length || 0, 'records')
  ...
  if (productionData) {
    ...
    console.log('✅ Success rates calculated:', successRate.value)
  }
  ...
  if (salesData) {
    ...
    console.log('💰 Revenue calculated:', summary.value.pendapatan)
  }
}
...
const initFaseChart = async () => {
  ...
  if (!canvas) {
    console.warn('⚠️ faseChart canvas not found')
    return
  }
...
const initBatchComparisonChart = async () => {
  ...
  if (!canvas) {
    console.warn('⚠️ batchComparisonChart canvas not found')
    return
  }

  if (batchList.value.length === 0) {
    console.warn('⚠️ No batches to display in chart')
    return
  }
...
const initPenjualanChart = async () => {
  ...
  if (!canvas) {
    console.warn('⚠️ penjualanChart canvas not found')
    return
  }
...
<template>
  <div class="min-h-screen bg-gradient-to-br from-gray-50 to-white">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <!-- Batch Cards dengan Button Lihat Detail -->
      <div class="mb-8">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          >
            <div class="flex items-start justify-between mb-4">
              <div class="flex-1">
                <h3 class="text-lg font-bold text-gray-900 mb-1">{{ batch.batch_name }}</h3>
                <p class="text-xs text-gray-500 mb-2">
                  {{ batch.tanggal_mulai ? new Date(batch.tanggal_mulai).toLocaleDateString('id-ID') : '-' }}
                </p>
              </div>
              <div class="w-12 h-12 bg-gradient-to-br from-[#0071f3] to-[#8FABD4] rounded-xl flex items-center justify-center text-white font-bold text-lg flex-shrink-0">
