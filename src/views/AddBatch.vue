...
<template>
  <div class="min-h-screen bg-[#FFFD8F] px-6 py-10 text-[#2F5320] flex flex-col justify-between">
    <div>
      <router-link
        to="/dashboard"
      ...
      <div class="max-w-lg mx-auto bg-white rounded-2xl shadow-lg p-8 border border-[#CFE9A8]">
        <form @submit.prevent="saveBatch" class="space-y-6">
          ...
          <div class="text-center mt-6">
            <button
              type="submit"
              class="bg-[#4C763B] text-white px-8 py-3 rounded-lg font-bold text-lg hover:bg-[#3b5c2f] transition"
            >
              💾 Simpan Batch
            </button>
          </div>
