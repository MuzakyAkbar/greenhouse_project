...
const router = createRouter({
  ...
  routes: [
    ...
    {
      path: "/reportProductionEdit/:record_id",
    ...
    // Parameter diubah dari :summary_id menjadi :record_id untuk mendukung approval Repair
    {
      ...
      component: () => import("../views/DamageReportReview.vue"),
      meta: { requiresAuth: true },
      beforeEnter: (to, from, next) => {
        if (!to.params.record_id) {
          alert('⚠️ Record ID tidak valid')
          next({ name: 'planningReportList' })
        } else {
          next()
        }
    ...
    // View dan Edit juga disesuaikan untuk menerima record_id jika diperlukan untuk konsistensi
    {
      ...
      component: () => import("../views/DamageReportView.vue"),
      meta: { requiresAuth: true },
    },
    {
      path: "/damageReportEdit/:record_id",
